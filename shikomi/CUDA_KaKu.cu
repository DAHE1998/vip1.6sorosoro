/* shikomi/CUDA_KaKu.cu — 一次解码出全分辨率关键帧（unified_extract）全部 CUDA kernel 合并版
 *
 * 用法: nvcc -arch=all -fatbin -o CUDA_KaKu.cubin CUDA_KaKu.cu（编译生成 .cubin 供 kaiseki 加载）
 * 依赖: CUDA 工具链（nvcc）；合并来源：shikomi_kernels.cu 全文（scdet_sad/blackdetect_count/
 ...
 *       （追加；nv12_rgb224 已在 shikomi_kernels.cu，不重复定义）
 * 产物: CUDA_KaKu.cubin
 */
